...
void Countdown::runPrintToFile() {
  ...
  // readHeader.open("bin/header_template.html", std::ios::in);
  // readFooter.open("bin/footer_template.html", std::ios::in);

  // void buildFile();
  // if (readHeader) {
  //   std::string line;
  //
  //   // Using getline below keeps white space and new line characters
  //   while(getline(readHeader, line)) {
  //     file.push_back(line + "\n");
  //   }
  //
  //   readHeader.close();
  //
  //   // Open Game datafile for writing
  //   datafile.open("../games/" + filename, std::ios::app);
  //   if(datafile.fail()){
  //     head = 'B';
  //   } else {
  //     // Write Header Template
  //     for(std::string ele : file) datafile << ele;
  //
  //     // Write game data
